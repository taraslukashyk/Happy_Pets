            ...
            <article key={i} style={{
              ...
              {s.featured && (
                <div style={{ position:'absolute', top:16, right:16 }}>
                  <span style={{ padding:'4px 10px', borderRadius:4, background:'#fff',
                    color:'var(--hp-rose)', fontSize:10, fontWeight:800,
                    letterSpacing:'0.14em', textTransform:'uppercase' }}>Рекомендовано</span>
                </div>
              )}
              ...
              <div style={{ fontFamily:'var(--hp-font-display)', fontWeight:700, fontSize:26,
                letterSpacing:'-0.015em', lineHeight:1.1 }}>{s.t}</div>
              <div style={{ fontSize:14, lineHeight:1.5,
                color: s.featured ? 'rgba(255,255,255,0.86)' : 'var(--hp-ink-2)' }}>{s.d}</div>
              <div style={{
                ...
                <div>
                  <div style={{ fontFamily:'var(--hp-font-display)', fontWeight:700,
                    fontSize:30, letterSpacing:'-0.02em' }}>{s.p}</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:20 }}>
          {team.map((p, i) => (
            <div key={p.n} onClick={() => setActive(i)} style={{
              ...
              <div style={{ fontSize:14, marginTop:14, lineHeight:1.5,
                color: active === i ? 'rgba(255,255,255,0.88)' : 'var(--hp-ink-2)' }}>{p.bio}</div>
            </div>
          ))}
        </div>
      </div>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} style={{ borderTop:'1px dashed var(--hp-rule)' }}>
                  ...
                  <td style={{ padding:'16px 0', textAlign:'right', color:'var(--hp-ink-3)',
                    fontFamily:'var(--hp-font-mono)', fontSize:13 }}>{r[4]}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const Footer = () => (
  <footer id="contacts" style={{ background:'var(--hp-ink)', color:'#fff', padding:'64px 0 32px' }}>
    ...
    <div style={{ borderTop:'1px solid rgba(255,255,255,0.12)', marginTop:40, paddingTop:20,
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          ...
          <button onClick={onClose} style={{
            ...
            background:'#fff', cursor:'pointer', fontSize:16, color:'var(--hp-ink-2)' }}>×</button>
        </div>
